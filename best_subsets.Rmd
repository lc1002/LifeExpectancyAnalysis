...
sub_fit = regsubsets(le ~ year + status + adult_mort + infantdeaths + alcohol + percent_exp + hepatitis_b + measles + bmi + under_five + polio + total_exp + diphtheria + hiv_aids + gdp + population + thin_1_19 + thin_5_9 + HDI + schooling, data = le_df)
...
```{r}
best_subsets_8 = lm(le ~ adult_mort + infantdeaths + under_five + polio + hiv_aids +gdp + HDI + schooling, data = le_df)
...
best_8 = lm(le ~ adult_mort + infantdeaths + polio + hiv_aids +gdp + HDI + schooling, data = le_df)
summary(best_8)
...
model = train(le ~ adult_mort + infantdeaths + polio + hiv_aids +gdp + HDI + schooling, data = le_df, trControl = train, method = 'lm', na.action = na.pass)
...
```{r}
best_subsets_7 = lm(le ~ adult_mort + bmi + polio + hiv_aids + gdp + HDI + schooling, data = le_df)
...
#remove high correlation variables
best_7 = lm(le ~ adult_mort + bmi + polio + hiv_aids + gdp + HDI + schooling, data = le_df)
summary(best_7)
...
model = train(le ~ adult_mort + bmi + polio + hiv_aids + gdp + HDI + schooling, data = le_df, trControl = train, method = 'lm', na.action = na.pass)
